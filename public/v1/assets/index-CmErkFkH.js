...
function FaceEnrollmentCrop({ candidateId, label }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { role: "img", "aria-label": label, viewBox: `${x2} ${y2} ${size} ${size}`, preserveAspectRatio: "xMidYMid slice", style: { width: "100%", height: "100%", display: "block" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("image", { href: FACE_ENROLLMENT_DEMO_IMAGE, x: "0", y: "0", width: FACE_ENROLLMENT_DEMO_SIZE.width, height: FACE_ENROLLMENT_DEMO_SIZE.height, preserveAspectRatio: "none" }) });
}
function App$1() {
  var _a;
  ...
  const [faceBatchCoachmarkVisible, setFaceBatchCoachmarkVisible] = reactExports.useState(false);
  const [faceBatchCoachmarkDismissed, setFaceBatchCoachmarkDismissed] = reactExports.useState(false);
  const [faceEnrollmentIntroOpen, setFaceEnrollmentIntroOpen] = reactExports.useState(true);
  const [faceEnrollmentGuideTarget, setFaceEnrollmentGuideTarget] = reactExports.useState(null);
  ...
  const faceEnrollAllButtonRef = reactExports.useRef(null);
  ...
  const startFaceBatchEnrollment = () => {
    ...
    faceEnrollTimerRef.current = setTimeout(() => {
      setTrk((p2) => {
        ...
        const availableSlots = Math.max(0, 20 - p2.enrolledFaces.length);
  ...
  reactExports.useEffect(() => {
    const shouldShow = trk.faceEnrollmentMode === "select" && faceSelectFlow.stage === "ready" && !faceSelectCoachmarkDismissed && trk.enrolledFaces.length < 20;
    if (!shouldShow) {
  ...
  reactExports.useEffect(() => {
    const shouldShow = trk.faceEnrollmentMode === "all" && trk.faceCaptureState !== "loading" && !faceEnrollmentIntroOpen && !faceBatchCoachmarkDismissed && trk.enrolledFaces.length < 20;
    if (!shouldShow) {
      setFaceBatchCoachmarkVisible(false);
      return void 0;
    }
  ...
  const changeFaceEnrollmentMode = (mode) => {
    ...
    setTrk((p2) => ({ ...p2, faceEnrollmentMode: mode, faceCaptureState: p2.faceCaptureState === "loading" ? "idle" : p2.faceCaptureState }));
  };
  const startFaceSelectEnrollment = (candidateId) => {
    ...
    faceSelectTimersRef.current = [
      ...
      setTimeout(() => {
        ...
        setTrk((p2) => {
          if (p2.enrolledFaces.length >= 20) return p2;
          const nextFace = {
            id: Date.now(),
            name: "",
            priority: p2.enrolledFaces.length + 1,
            candidateId
          };
          return { ...p2, enrolledFaces: [...p2.enrolledFaces, nextFace] };
        });
      }, 2100),
  ...
  const [draggedLivePresetId, setDraggedLivePresetId] = reactExports.useState(null);
  const [livePresetDropActive, setLivePresetDropActive] = reactExports.useState(false);
  const [livePresetDragOverlay, setLivePresetDragOverlay] = reactExports.useState(null);
  const livePresetPointerDragRef = reactExports.useRef(null);
  const suppressLivePresetClickRef = reactExports.useRef(false);
  ...
  const moveLivePresetPointerDrag = (event) => {
    ...
    const previewRect = (_a2 = document.getElementById("aver-live-preview-panel")) == null ? void 0 : _a2.getBoundingClientRect();
    const overPreview = Boolean(previewRect && event.clientX >= previewRect.left && event.clientX <= previewRect.right && event.clientY >= previewRect.top && event.clientY <= previewRect.bottom);
  ...
  const finishLivePresetPointerDrag = (event, allowDrop = true) => {
    ...
    if (!drag || drag.pointerId !== event.pointerId) return;
    const previewRect = (_a2 = document.getElementById("aver-live-preview-panel")) == null ? void 0 : _a2.getBoundingClientRect();
    const droppedOnPreview = Boolean(allowDrop && drag.dragging && previewRect && event.clientX >= previewRect.left && event.clientX <= previewRect.right && event.clientY >= previewRect.top && event.clientY <= previewRect.bottom);
    (_c = (_b = event.currentTarget).releasePointerCapture) == null ? void 0 : _c.call(_b, event.pointerId);
  ...
  reactExports.useEffect(() => {
    const handleMouseMove = (event) => {
      ...
      const previewRect = (_a2 = document.getElementById("aver-live-preview-panel")) == null ? void 0 : _a2.getBoundingClientRect();
      setLivePresetDropActive(Boolean(previewRect && event.clientX >= previewRect.left && event.clientX <= previewRect.right && event.clientY >= previewRect.top && event.clientY <= previewRect.bottom));
    };
    const handleMouseUp = (event) => {
      ...
      if (!drag || drag.pointerId !== "mouse") return;
      const previewRect = (_a2 = document.getElementById("aver-live-preview-panel")) == null ? void 0 : _a2.getBoundingClientRect();
      const droppedOnPreview = Boolean(drag.dragging && previewRect && event.clientX >= previewRect.left && event.clientX <= previewRect.right && event.clientY >= previewRect.top && event.clientY <= previewRect.bottom);
      livePresetPointerDragRef.current = null;
      setDraggedLivePresetId(null);
      setLivePresetDropActive(false);
      setLivePresetDragOverlay(null);
      ...
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              ...
              children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, children: "V1 · 08/10" }),
          ...
          versionMenuOpen && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-version-switcher-menu", role: "listbox", "aria-label": "Prototype versions", style: { position: "absolute", top: 38, left: 0, width: "100%", boxSizing: "border-box", padding: 4, borderRadius: 6, border: `1px solid ${T.line2}`, background: T.panel2, boxShadow: "0 10px 28px rgba(0,0,0,0.46)", overflow: "hidden" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: "aver-version-option-v1", type: "button", role: "option", "aria-selected": "true", onClick: () => setVersionMenuOpen(false), style: { width: "100%", minHeight: 32, padding: "6px 9px", border: "none", borderRadius: 4, background: "rgba(30,155,240,0.16)", color: "#fff", fontFamily: fUI, fontSize: 11.5, fontWeight: 600, textAlign: "left", cursor: "pointer", boxShadow: `inset 2px 0 0 ${T.blue}` }, children: "V1 · 08/10" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: "aver-version-option-v2", type: "button", role: "option", "aria-selected": "false", onClick: () => window.location.assign("../v2/index.html"), onMouseEnter: (event) => {
              event.currentTarget.style.background = "rgba(255,255,255,0.06)";
            }, onMouseLeave: (event) => {
              event.currentTarget.style.background = "transparent";
            }, style: { width: "100%", minHeight: 32, padding: "6px 9px", border: "none", borderRadius: 4, background: "transparent", color: T.dim, fontFamily: fUI, fontSize: 11.5, textAlign: "left", cursor: "pointer" }, children: "V2 · 08/10" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: "aver-version-option-v3", type: "button", role: "option", "aria-selected": "false", onClick: () => window.location.assign("../index.html"), onMouseEnter: (event) => {
              ...
              event.currentTarget.style.background = "transparent";
            }, style: { width: "100%", minHeight: 32, padding: "6px 9px", border: "none", borderRadius: 4, background: "transparent", color: T.dim, fontFamily: fUI, fontSize: 11.5, textAlign: "left", cursor: "pointer" }, children: "V3 · 08/11" })
          ] })
        ]
      }
    ),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-main-stage", style: { position: "relative", flex: 1, padding: "16px 24px", minWidth: 0, background: T.page, overflow: "hidden", height: "100vh", display: "flex", flexDirection: "column", boxSizing: "border-box" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "aver-page-transition", style: { width: "100%", height: "100%", minHeight: 0 }, children: [
        activeMenu === "paint" ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-content-wrapper", style: { display: "flex", flexDirection: "column", gap: SP[2], width: "min(calc(75vw - 40px), 100%)", marginLeft: "max(0px, calc(16.6667vw - 225.33px))", height: "100%", minHeight: 0 }, children: paintLayout === "classic" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "aver-classic-layout-entrance", style: { display: "flex", flexDirection: "column", gap: 10, width: "100%", height: "100%", minHeight: 0 }, children: [
          ...
          return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { position: "relative", height: "100%", width: "auto", aspectRatio: "16 / 9", overflow: "hidden" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-live-preset-active-preview", "data-preset-id": live.selectedQuickCall ?? 0, style: { position: "absolute", inset: 0, backgroundImage: `url(${live.activePreviewImage})`, backgroundSize: "cover", backgroundPosition: "center", transform: `translate(${ptz.pan}%, ${ptz.tilt}%) scale(${ptz.zoom * 1.65})`, transition: "transform 0.1s ease-out, opacity 0.18s ease" } }) }),
                  draggedLivePresetId != null && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-live-preset-drop-hint", role: "status", style: { position: "absolute", inset: 0, zIndex: 5, pointerEvents: "none", display: "flex", alignItems: "center", justifyContent: "center", background: livePresetDropActive ? "rgba(8,15,23,0.42)" : "rgba(0,0,0,0.16)", transition: "background 0.16s ease" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { padding: "9px 14px", borderRadius: 7, border: `1px solid ${T.blue}`, background: "rgba(10,14,19,0.92)", color: "#fff", fontSize: 13, fontWeight: 700, boxShadow: "0 8px 22px rgba(0,0,0,0.45)" }, children: livePresetDropActive ? `Release to load ${live.presetNames[draggedLivePresetId]}` : `Drag ${live.presetNames[draggedLivePresetId]} here` }) })
                ]
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-live-control-panel", style: { background: T.panel, border: `1px solid ${T.line}`, borderRadius: 10, flex: "0 0 300px", height: 300, display: "flex", flexDirection: "column", overflow: "hidden" }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", borderBottom: `1px solid ${T.line}` }, children: [
                [["control", "Camera Control"], ["preset", "Preset"]].map(([id2, lb2]) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                ...
                live.tab === "preset" && /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
              ...
              live.tab === "control" ? (
                ...
                /* ===== Preset(預設位置)===== */
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-live-preset-subpanel", style: { padding: "10px 12px", flex: 1, minHeight: 0, overflow: "auto", boxSizing: "border-box" }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-live-preset-layout-row", "data-layout": live.presetLayoutVariant, style: { display: "flex", gap: 10, minWidth: 0, height: "100%", minHeight: 0 }, children: [
                  ...
                  live.presetLayoutVariant === "separate" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-live-preset-save-options", style: { flex: "1.2 1 0", minWidth: 218, display: "flex", flexDirection: "column", gap: 6 }, children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: presetSection, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: presetHeader, children: "Save Preset" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 7, padding: 7 }, children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("input", { id: "aver-live-preset-save-number", "aria-label": "Save preset number", type: "number", min: "0", max: "19", value: live.presetSaveNumber, onChange: (e) => updLive("presetSaveNumber", e.target.value), style: { ...presetInput, flex: 1 } }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: "aver-live-preset-save-button", type: "button", onClick: saveLivePreset, style: presetButton, children: "Save" })
                      ] })
                    ] }),
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-live-preset-load-options", style: { ...presetSection, flex: live.presetLayoutVariant === "integrated" ? "3.2 1 0" : "2 1 0", minWidth: 360, display: "flex", flexDirection: "column" }, children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: presetHeader, children: "Preset Library" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-live-preset-library-body", "data-layout": live.presetLayoutVariant, style: { flex: 1, minHeight: 0, display: "flex", alignItems: "stretch", gap: live.presetLayoutVariant === "integrated" ? 8 : 0 }, children: [
                      live.presetLayoutVariant === "integrated" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-live-preset-integrated-save-panel", style: { flex: "1 1 0", minWidth: 170, padding: 7, display: "flex", flexDirection: "column", justifyContent: "center", gap: 6, borderRight: `1px solid ${T.line}`, background: "rgba(255,255,255,0.018)", boxSizing: "border-box" }, children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { ...presetSection, padding: 7 }, children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginBottom: 4, color: T.faint, fontSize: 10.5, fontWeight: 600 }, children: "Save Preset" }),
                          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", gap: 6 }, children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsx("input", { id: "aver-live-preset-integrated-save-number", "aria-label": "Integrated layout save preset number", type: "number", min: "0", max: "19", value: live.presetSaveNumber, onChange: (event) => updLive("presetSaveNumber", event.target.value), style: { ...presetInput, height: 27, flex: 1 } }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: "aver-live-preset-integrated-save-button", type: "button", onClick: saveLivePreset, style: { ...presetButton, height: 27, padding: "0 11px" }, children: "Save" })
                          ] })
                        ] }),
                        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { id: "aver-live-preset-integrated-video-freeze-option", style: { ...presetSection, minHeight: 32, padding: "0 8px", display: "flex", alignItems: "center", gap: 6, color: T.text, fontSize: 11, cursor: "pointer", whiteSpace: "nowrap" }, children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx("input", { type: "checkbox", checked: live.presetVideoFreeze, onChange: (event) => updLive("presetVideoFreeze", event.target.checked) }),
                          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Video Freeze" })
                        ] }),
                        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { id: "aver-live-preset-integrated-accuracy-option", style: { ...presetSection, minHeight: 32, padding: "0 8px", display: "flex", alignItems: "center", gap: 6, color: T.text, fontSize: 11, cursor: "pointer", whiteSpace: "nowrap" }, children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx("input", { type: "checkbox", checked: live.presetAccuracy, onChange: (event) => updLive("presetAccuracy", event.target.checked) }),
                          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Preset Accuracy" })
                        ] }),
                        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-live-preset-integrated-speed-option", style: { ...presetSection, minWidth: 0, padding: "7px 8px" }, children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", color: T.faint, fontSize: 10.5, fontWeight: 600 }, children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Preset Speed" }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: T.text, fontFamily: fMono }, children: live.presetSpeed })
                          ] }),
                          /* @__PURE__ */ jsxRuntimeExports.jsx("input", { "aria-label": "Integrated layout preset speed", type: "range", min: "5", max: "200", value: live.presetSpeed, onChange: (event) => updLive("presetSpeed", Number(event.target.value)), style: { width: "100%", margin: "5px 0 0", accentColor: T.blue } }),
                          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", color: T.faint, fontSize: 10, marginTop: 1 }, children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "5" }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "200" })
                          ] })
                        ] })
                      ] }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-live-preset-library-cards-column", style: { flex: live.presetLayoutVariant === "integrated" ? "3 1 0" : "1 1 0", minWidth: 0, padding: "6px 7px 7px", display: "flex", flexDirection: "column", gap: 6 }, children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { minHeight: 25, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }, children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { minWidth: 0, color: T.faint, fontSize: 11.5, lineHeight: 1.35 }, children: "Select a preset card to load it automatically." }),
                          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: "aver-live-preset-edit-scenes-button", type: "button", onClick: () => setToast("Edit Scenes is ready"), style: { ...presetButton, height: 25, padding: "0 10px", flexShrink: 0 }, children: "Edit Scenes" })
                        ] }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-live-preset-card-scroll", style: { flex: live.presetLayoutVariant === "integrated" ? 1 : "0 1 auto", minHeight: 0, maxHeight: live.presetLayoutVariant === "integrated" ? "none" : 198, overflowY: "auto", paddingRight: 3, scrollbarColor: `${T.line2} transparent` }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-live-preset-quick-call-grid", style: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(108px, 1fr))", gap: 7 }, children: Array.from({ length: 20 }, (_, n2) => {
                          const snapshot = live.presetSnapshots[n2];
                          const saved = live.savedPresetIds.includes(n2) && Boolean(snapshot);
                          const selected = saved && live.selectedQuickCall === n2;
                          ...
                          return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                            "div",
                            {
                              ...
                              onMouseDown: (event) => {
                                if (saved) startLivePresetMouseDrag(event, n2);
                              },
                              onPointerDown: (event) => {
                                if (saved) startLivePresetPointerDrag(event, n2);
                              },
                              ...
                              style: { minWidth: 0, overflow: "hidden", borderRadius: 5, border: `${selected ? 2 : 1}px ${draggedLivePresetId === n2 ? "dashed" : "solid"} ${selected || draggedLivePresetId === n2 ? T.blue : T.line}`, background: draggedLivePresetId === n2 ? "rgba(30,155,240,0.05)" : selected ? "rgba(23,145,236,0.12)" : "#101216", boxShadow: selected && draggedLivePresetId !== n2 ? "0 0 0 1px rgba(23,145,236,0.30)" : "none", touchAction: saved ? "none" : "auto", userSelect: "none", cursor: saved ? draggedLivePresetId === n2 ? "grabbing" : "grab" : "default", opacity: draggedLivePresetId === n2 ? 0.18 : 1, transition: "opacity 0.16s ease, background 0.16s ease, border-color 0.16s ease, box-shadow 0.16s ease" },
                              children: [
                                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                                  "button",
                                  {
                                    ...
                                    type: "button",
                                    "aria-label": saved ? `Load ${presetName}` : `Preset ${n2} is empty`,
                                ...
                                ) : /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: `aver-live-preset-name-${n2}`, type: "button", title: "Click name to edit", onClick: () => beginLivePresetRename(n2), style: { width: "100%", height: 27, minWidth: 0, padding: "0 6px", display: "flex", alignItems: "center", cursor: "text", border: "none", background: "transparent", color: selected ? "#fff" : T.text, fontFamily: fUI, fontSize: 11.5, textAlign: "left" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, children: presetName }) })
                              ]
                            },
                            n2
                          );
            ...
            livePresetDragOverlay && (() => {
              ...
              return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { position: "relative", width: "100%", aspectRatio: "16 / 9", overflow: "hidden", borderBottom: `1px solid ${T.line}` }, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { "aria-hidden": "true", style: { position: "absolute", inset: 0, backgroundImage: `linear-gradient(rgba(0,0,0,0.04), rgba(0,0,0,0.16)), url(${overlaySnapshot.image})`, backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat", transform: `translate(${overlaySnapshot.pan}%, ${overlaySnapshot.tilt}%) scale(${overlaySnapshot.zoom * 1.65})`, transformOrigin: "center" } }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { position: "absolute", zIndex: 1, left: 5, top: 4, minWidth: 20, height: 18, padding: "0 4px", boxSizing: "border-box", display: "inline-flex", alignItems: "center", justifyContent: "center", borderRadius: 3, background: "rgba(0,0,0,0.76)", color: "#fff", fontFamily: fMono, fontSize: 10, fontWeight: 700 }, children: String(overlayPresetId).padStart(2, "0") })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { width: "100%", height: 27, boxSizing: "border-box", padding: "0 6px", display: "flex", alignItems: "center", color: T.text, fontFamily: fUI, fontSize: 11.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }, children: live.presetNames[overlayPresetId] })
                  ]
                }
              );
            })()
          ...
          return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-tracking-wrapper", style: { width: "min(calc(75vw - 40px), 100%)", marginLeft: "max(0px, calc(16.6667vw - 225.33px))", height: "100%", minHeight: 0, overflow: "hidden", paddingRight: 8, boxSizing: "border-box", display: "flex", flexDirection: "column", gap: 10 }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { flex: "1 1 0", minHeight: 0, display: "flex", gap: 10, alignItems: "stretch" }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-trk-preview-panel", style: { flex: "1 1 0", minWidth: 0, position: "relative", borderRadius: 10, overflow: "hidden", border: `1px solid ${T.line}`, background: "#000", minHeight: 0 }, children: [
                trk.tab === "face" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { id: "aver-face-enrollment-live-view", viewBox: `0 0 ${FACE_ENROLLMENT_DEMO_SIZE.width} ${FACE_ENROLLMENT_DEMO_SIZE.height}`, preserveAspectRatio: "xMidYMid meet", style: { width: "100%", height: "100%", display: "block", transform: faceSelectIsFocused ? faceSelectFocusedTransform : `translate(${ptz.pan}%, ${ptz.tilt}%) scale(${ptz.zoom})`, transformOrigin: faceSelectIsFocused ? faceSelectOrigin : "50% 50%", transition: "transform 1.2s cubic-bezier(0.45, 0, 0.55, 1), transform-origin 1.2s cubic-bezier(0.45, 0, 0.55, 1)" }, children: [
                  ...
                  (trk.faceEnrollmentMode === "select" ? faceSelectFlow.stage !== "initializing" : trk.faceCaptureState === "complete") && FACE_ENROLLMENT_CANDIDATES.map((candidate) => {
                    ...
                    const isAdded = (_b = (_a2 = trk.faceBatchResult) == null ? void 0 : _a2.addedCandidateIds) == null ? void 0 : _b.includes(candidate.id);
                    ...
                    const label = blockedByLibraryFull ? "Library full" : isSelectMode ? isValidSelectFace ? isSelected && faceSelectStageLabel ? faceSelectStageLabel.replace("...", "") : isHovered ? "Enroll this face" : "＋ Enroll" : candidate.label : isAdded ? "Enrolled" : isValidSelectFace ? "Eligible" : candidate.label;
                    const { x: x2, y: y2, size } = candidate.crop;
                    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      "g",
                      {
                        ...
                        "aria-label": isSelectMode ? isValidSelectFace ? `Enroll ${candidate.id}` : `${candidate.label}; face cannot be enrolled` : void 0,
                        ...
                        children: [
                          shouldPulse && /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { className: "aver-face-select-pulse", x: x2 - 7, y: y2 - 7, width: size + 14, height: size + 14, fill: "none", stroke: T.blue, vectorEffect: "non-scaling-stroke" }),
                          /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: x2, y: y2, width: size, height: size, fill: isHovered ? "rgba(23,145,236,0.13)" : isSelectMode ? "rgba(23,145,236,0.001)" : "none", stroke: color, strokeWidth: isHovered ? "8" : "5", vectorEffect: "non-scaling-stroke" }),
                          /* @__PURE__ */ jsxRuntimeExports.jsx("rect", { x: x2, y: Math.max(0, y2 - 30), width: Math.max(96, label.length * 15), height: "30", fill: color, opacity: "0.94" }),
                          /* @__PURE__ */ jsxRuntimeExports.jsx("text", { x: x2 + 9, y: Math.max(21, y2 - 9), fill: "#fff", fontSize: "20", fontWeight: "700", fontFamily: fUI, children: label })
                        ]
                      },
                      candidate.id
                    );
                ...
                trk.tab === "face" && trk.faceEnrollmentMode === "all" && faceBatchCoachmarkVisible && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enroll-all-coachmark", role: "status", "aria-live": "polite", style: { position: "absolute", top: 12, left: "50%", transform: "translateX(-50%)", zIndex: 4, minHeight: 36, maxWidth: "calc(100% - 28px)", boxSizing: "border-box", padding: "7px 9px 7px 12px", display: "flex", alignItems: "center", gap: 8, borderRadius: 7, border: "1px solid rgba(30,155,240,0.58)", background: "rgba(8,12,17,0.92)", color: "#fff", boxShadow: "0 8px 24px rgba(0,0,0,0.38)", fontSize: 12.5, fontWeight: 600, whiteSpace: "nowrap" }, children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { "aria-hidden": "true", style: { color: T.blue, fontSize: 16, lineHeight: 1 }, children: "☝" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Use Enroll All Faces to add every eligible face" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: "aver-face-enroll-all-coachmark-dismiss-button", type: "button", "aria-label": "Dismiss enroll all faces tip", onClick: () => {
                    setFaceBatchCoachmarkVisible(false);
                    setFaceBatchCoachmarkDismissed(true);
                  }, style: { width: 22, height: 22, padding: 0, marginLeft: 2, display: "inline-flex", alignItems: "center", justifyContent: "center", borderRadius: 4, border: "none", background: "transparent", color: T.dim, fontFamily: fUI, fontSize: 16, cursor: "pointer" }, children: "×" })
                ] }),
                trk.tab === "face" && trk.faceEnrollmentMode === "select" && faceSelectCoachmarkVisible && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-select-coachmark", role: "status", "aria-live": "polite", style: { position: "absolute", top: 12, left: "50%", transform: "translateX(-50%)", zIndex: 4, minHeight: 36, maxWidth: "calc(100% - 28px)", boxSizing: "border-box", padding: "7px 9px 7px 12px", display: "flex", alignItems: "center", gap: 8, borderRadius: 7, border: "1px solid rgba(30,155,240,0.58)", background: "rgba(8,12,17,0.92)", color: "#fff", boxShadow: "0 8px 24px rgba(0,0,0,0.38)", fontSize: 12.5, fontWeight: 600, whiteSpace: "nowrap" }, children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { "aria-hidden": "true", style: { color: T.blue, fontSize: 16, lineHeight: 1 }, children: "☝" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Select a blue face to enroll" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: "aver-face-select-coachmark-dismiss-button", type: "button", "aria-label": "Dismiss face selection tip", onClick: () => {
                    setFaceSelectCoachmarkVisible(false);
                    setFaceSelectCoachmarkDismissed(true);
                  }, style: { width: 22, height: 22, padding: 0, marginLeft: 2, display: "inline-flex", alignItems: "center", justifyContent: "center", borderRadius: 4, border: "none", background: "transparent", color: T.dim, fontFamily: fUI, fontSize: 16, cursor: "pointer" }, children: "×" })
                ] }),
                trk.tab === "face" && trk.faceCaptureState === "loading" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-loading", role: "status", "aria-live": "polite", style: { position: "absolute", inset: 0, background: "rgba(5,7,9,0.68)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, color: "#fff", zIndex: 2 }, children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: 14, fontWeight: 600 }, children: "Detecting and enrolling faces..." })
                ] }),
                trk.tab === "face" && trk.faceEnrollmentMode === "select" && faceSelectStageLabel && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-select-progress", role: "status", "aria-live": "polite", style: { position: "absolute", top: 12, left: "50%", transform: "translateX(-50%)", display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 6, background: "rgba(8,10,13,0.88)", border: `1px solid ${T.line2}`, color: "#fff", fontSize: 12.5, fontWeight: 600, zIndex: 3, pointerEvents: "none" }, children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-trk-control-panel", style: { flex: "0 0 350px", height: 350, minHeight: 0, background: T.panel, border: `1px solid ${T.line}`, borderRadius: 10, display: "flex", flexDirection: "column", overflow: "hidden" }, children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { flex: 1, minHeight: 0, overflowY: "auto", padding: 8 }, children: trk.tab === "presenter" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, alignItems: "start" }, children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment", style: { height: "100%", minHeight: 0, display: "flex" }, children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-mode-panel", style: { flex: "0 0 180px", minWidth: 0, boxSizing: "border-box", padding: "4px 12px 4px 4px", borderRight: `1px solid ${T.line}`, display: "flex", flexDirection: "column", gap: 9 }, children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-mode-header", style: { display: "flex", alignItems: "center", gap: 6 }, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: 13.5, fontWeight: 600, color: T.text }, children: "Enrollment Mode" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "button",
                        {
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", flexDirection: "column", gap: 6 }, children: [
                      ...
                      return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                        "button",
                        {
                          ...
                          children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { "aria-hidden": "true", style: { width: 12, height: 12, flexShrink: 0, borderRadius: "50%", boxSizing: "border-box", border: `2px solid ${active ? T.blue : T.faint}`, boxShadow: active ? `inset 0 0 0 2px #101216` : "none", background: active ? T.blue : "transparent" } }),
                            label
                          ]
                        },
                        mode
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-face-enrollment-mode-description", style: { fontSize: 11.5, color: T.faint, lineHeight: 1.45 }, children: trk.faceEnrollmentMode === "all" ? "Enroll every detected face in one batch." : "Click a blue face box to frame and enroll that person. Red boxes indicate faces that cannot be enrolled." }),
                    trk.faceEnrollmentMode === "all" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "button",
                      {
                        ...
                        children: trk.faceCaptureState === "loading" ? "Enrolling..." : "Enroll All Faces"
                      }
                    )
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-enrolled-face-panel", style: { flex: "1 1 0", minWidth: 0, minHeight: 0, paddingLeft: 12, display: "flex", flexDirection: "column", gap: 8 }, children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-header", style: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { id: "aver-enrolled-face-count", style: { fontSize: 16, fontWeight: 500, color: T.text }, children: [
                        "Enrolled Face (",
                        trk.enrolledFaces.length,
                        "/20)"
                      ] }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", alignItems: "center", gap: 10 }, children: [
                        trk.enrolledFaces.length >= 20 && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { id: "aver-enrolled-face-library-full-status", role: "status", style: { padding: "4px 8px", borderRadius: 4, border: "1px solid rgba(245,166,35,0.42)", background: "rgba(245,166,35,0.10)", color: "#f5b74f", fontSize: 11.5, fontWeight: 600 }, children: "Face library is full" }),
                              ...
                              "div",
                              {
                                ...
                                children: [
                                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: `aver-enrolled-face-photo-${order}`, "aria-label": `Enrolled face ${index + 1}`, style: { width: 92, height: 92, boxSizing: "border-box", position: "relative", overflow: "hidden", border: `1px solid ${isEditing ? T.blue : T.line2}`, backgroundColor: "rgba(23,145,236,0.12)" }, children: [
                                    /* @__PURE__ */ jsxRuntimeExports.jsx(FaceEnrollmentCrop, { candidateId: face.candidateId, label: `Face ${index + 1} photo` }),
                                    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { id: `aver-enrolled-face-number-${order}`, title: `Priority ${order}`, "aria-label": `Priority ${order}`, style: { position: "absolute", top: 3, left: 3, padding: "2px 4px", borderRadius: 3, background: "rgba(8,10,13,0.74)", color: "#fff", fontSize: 10, fontWeight: 700, lineHeight: 1, textShadow: "0 1px 3px #000" }, children: [
                                      "P",
                                      order
                                    ] }),
                                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                                  ...
                                  isEditing ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: `aver-enrolled-face-name-edit-state-${order}`, style: { display: "flex", flexDirection: "column", gap: 2 }, children: [
                                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: 9.5, color: T.blue, fontWeight: 600, lineHeight: 1 }, children: "Editing" }),
                                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                                      "input",
                                      {
                ...
                faceEnrollmentIntroOpen && faceEnrollmentGuideTarget && (() => {
                  ...
                  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-first-run-guide", "aria-label": "Face enrollment first-step guide", style: { position: "fixed", inset: 0, zIndex: 70, pointerEvents: "none" }, children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { ...mask, left: 0, top: 0, width: "100vw", height: top } }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { ...mask, left: 0, top, width: left, height: spotlightHeight } }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { ...mask, left: right, top, right: 0, height: spotlightHeight } }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { ...mask, left: 0, top: bottom, width: "100vw", bottom: 0 } }),
                    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-face-enrollment-guide-spotlight", style: { position: "fixed", zIndex: 71, left, top, width: spotlightWidth, height: spotlightHeight, borderRadius: 8, border: `2px solid ${T.blue}`, boxSizing: "border-box", boxShadow: "0 0 0 3px rgba(30,155,240,0.22), 0 0 24px rgba(30,155,240,0.55)", pointerEvents: "none" } }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-guide-message", role: "status", "aria-live": "polite", style: { position: "fixed", zIndex: 71, left: left + spotlightWidth / 2, top: Math.max(14, top - 73), transform: "translateX(-50%)", width: "max-content", maxWidth: "min(300px, calc(100vw - 28px))", display: "flex", flexDirection: "column", alignItems: "center", gap: 2, color: "#fff", fontFamily: fUI, textAlign: "center", pointerEvents: "none" }, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { padding: "7px 11px", borderRadius: 7, border: "1px solid rgba(30,155,240,0.72)", background: "rgba(8,12,17,0.96)", boxShadow: "0 8px 20px rgba(0,0,0,0.45)", fontSize: 12.5, fontWeight: 700 }, children: "Start here: enroll detected faces" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "aver-face-enrollment-guide-arrow", "aria-hidden": "true", style: { color: T.blue, fontSize: 30, lineHeight: 0.8, textShadow: "0 0 12px rgba(30,155,240,0.85)" }, children: "↓" })
                    ] })
                  ] });
                })(),
                faceEnrollmentTourOpen && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-face-enrollment-tour-modal", role: "dialog", "aria-modal": "true", "aria-labelledby": "aver-face-enrollment-tour-title", style: { position: "fixed", inset: 0, zIndex: 75, padding: 24, background: "rgba(0,0,0,0.72)", display: "flex", alignItems: "center", justifyContent: "center" }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-tour-dialog", style: { width: "min(590px, calc(100vw - 48px))", overflow: "hidden", borderRadius: 10, border: `1px solid ${T.line2}`, background: "#101216", boxShadow: "0 22px 64px rgba(0,0,0,0.58)" }, children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-tour-header", style: { minHeight: 52, padding: "0 16px 0 20px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${T.line2}` }, children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 2, color: T.faint, fontSize: 11.5 }, children: "Understand face detection and enrollment priority." })
                    ] }),
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-tour-content", style: { padding: 20, display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 14 }, children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-tour-blue-frame", style: { minWidth: 0, padding: 12, borderRadius: 8, border: "1px solid rgba(30,155,240,0.32)", background: "rgba(30,155,240,0.06)" }, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { height: 146, padding: 10, boxSizing: "border-box", borderRadius: 6, background: "#090b0f", display: "flex", alignItems: "center", justifyContent: "center" }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { width: 116, height: 116, position: "relative", overflow: "hidden" }, children: [
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 10, color: T.blue, fontSize: 13, fontWeight: 700 }, children: "Blue frame · Ready to enroll" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 5, color: T.dim, fontSize: 12, lineHeight: 1.5 }, children: "The face is front-facing, clear, and large enough. Select it in Select Face mode, or include it with Enroll All Faces." })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-tour-red-frame", style: { minWidth: 0, padding: 12, borderRadius: 8, border: "1px solid rgba(239,83,80,0.30)", background: "rgba(239,83,80,0.05)" }, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { height: 146, padding: 10, boxSizing: "border-box", borderRadius: 6, background: "#090b0f", display: "flex", alignItems: "center", justifyContent: "center" }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { width: 116, height: 116, position: "relative", overflow: "hidden" }, children: [
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 10, color: "#ef6c68", fontSize: 13, fontWeight: 700 }, children: "Red frame · Cannot enroll" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 5, color: T.dim, fontSize: 12, lineHeight: 1.5 }, children: "The face may be angled, blurred, obstructed, or too small. Red-framed faces are excluded from enrollment." })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-tour-priority-order", style: { gridColumn: "1 / -1", minWidth: 0, padding: 12, borderRadius: 8, border: `1px solid ${T.line2}`, background: "rgba(255,255,255,0.025)", display: "flex", alignItems: "center", gap: 16 }, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-enrollment-tour-priority-example", "aria-label": "Example of enrolled face cards ordered by priority", style: { flex: "0 0 280px", minWidth: 0, padding: 8, borderRadius: 6, border: `1px solid ${T.line}`, background: "#171a1f" }, children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginBottom: 7, color: T.text, fontSize: 11.5, fontWeight: 600 }, children: "Enrolled Face (3/20)" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { display: "flex", alignItems: "flex-start", gap: 7 }, children: [
                          ...
                          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { width: 76, height: 70, position: "relative", overflow: "hidden", border: `1px solid ${T.line2}`, boxSizing: "border-box", background: "rgba(23,145,236,0.12)" }, children: [
                            ...
                            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { position: "absolute", top: 3, left: 3, padding: "2px 4px", borderRadius: 3, background: "rgba(8,10,13,0.78)", color: "#fff", fontSize: 9, fontWeight: 700, lineHeight: 1 }, children: card.priority }),
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { minWidth: 0 }, children: [
                        ...
                        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { marginTop: 4, color: T.dim, fontSize: 12, lineHeight: 1.45 }, children: "P01 is the highest priority. Drag a face card to another position to change its priority; the remaining cards will reorder automatically." })
                      ] })
                    ] })
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-face-enrollment-tour-footer", style: { padding: "12px 16px", display: "flex", justifyContent: "flex-end", borderTop: `1px solid ${T.line2}`, background: "rgba(255,255,255,0.018)" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: "aver-face-enrollment-tour-confirm-button", type: "button", onClick: () => setFaceEnrollmentTourOpen(false), style: { ...primaryBtn, minWidth: 92 }, children: "Got it" }) })
                ] }) }),
                faceDragOverlay && (() => {
                ...
                faceDeleteTarget && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "aver-face-delete-modal", role: "dialog", "aria-modal": "true", "aria-labelledby": "aver-face-delete-title", style: { position: "fixed", inset: 0, zIndex: 60, padding: 24, background: "rgba(0,0,0,0.72)", display: "flex", alignItems: "center", justifyContent: "center" }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { id: "aver-face-delete-dialog", style: { width: "min(390px, calc(100vw - 48px))", overflow: "hidden", background: "#101216", border: `1px solid ${T.line2}`, borderRadius: 10, boxShadow: "0 20px 60px rgba(0,0,0,0.55)" }, children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { padding: "0 18px 18px", color: T.dim, fontSize: 13, lineHeight: 1.55 }, children: [
                    ...
                    " will be permanently removed. The following cards will move forward automatically."
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "flex-end", gap: 9, padding: "12px 16px", borderTop: `1px solid ${T.line2}`, background: "rgba(255,255,255,0.02)" }, children: [
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx("button", { id: "aver-face-delete-confirm-button", type: "button", onClick: confirmFaceDelete, style: { ...primaryBtn, background: "#d94b4b" }, children: "Delete" })
                  ] })
                ] }) })
              ] }) })
            ] })
