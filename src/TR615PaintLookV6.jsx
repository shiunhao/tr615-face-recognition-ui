...
import "./tracking-typography.css";
...
import AudioIntegratedPanel, { createAudioIntegratedState } from "./AudioIntegratedPanel";
/* ============================================================================
...
export default function App({ prototypeVersion = "v2" }) {
  ...
  const [net, setNet] = useState({
    ...
    rtmpUrl: "", rtmpKey: "", rtspSec: "off", rtspUsername: "Admin", rtspPassword: "Admin", rtspAudio: "off", hlsUrl: "",
    srtIp: "", srtPort: "8889", srtEnc: "None", srtLatency: "1000", srtPass: "",
    https: "off", sshd: "off", viscaMode: "Default", viscaPort: "52381",
    dot1x: "off", eapMethod: "md5", eapIdentity: "239.255.0.0", eapPassword: "password", eapPrivateKeyPassword: "password", eapClientFile: "", eapCaFile: "",
    freeD: "off", freeDCameraId: "255", freeDIp: "192.168.1.168", freeDPort: "40000", freeDPanBacklash: "0", freeDTiltBacklash: "0",
  });
  const [savedNetworkAddress, setSavedNetworkAddress] = useState({ ip: "192.168.1.168", netmask: "255.255.255.0", gateway: "192.168.1.254", dns: "8.8.8.8" });
  const updNet = (k, v) => setNet((p) => ({ ...p, [k]: v }));
  // Tracking Settings 頁面狀態
  const microphoneInfoRef = useRef(null);
  const presenterHelpRef = useRef(null);
  const [presenterHelp, setPresenterHelp] = useState({ title: "", text: "" });
  const [savedTrackingArea, setSavedTrackingArea] = useState(null);
  const [audienceSettings, setAudienceSettings] = useState({ targets: { microphone: true, pose: false, hand: false }, sensitivity: 2, returnTime: 3, presetPoint: "1", peopleSize: "Full Body", placement: "Center", height: "Height1", multiPresenterTracking: false, effectiveArea: false, shieldZone: false, savedArea: null, savedPoint: null });
  const [trk, setTrk] = useState({
    ...
    microphoneTracking: false, multiPresenterTracking: false, multiPresenter: "off", shieldZone: false,
  ...
  const updTrk = (k, v) => setTrk((p) => ({
    ...p, [k]: v,
    ...(k === "microphoneTracking" && v ? { multiPresenter: "off" } : {}),
    ...(k === "multiPresenter" && v !== "off" ? { microphoneTracking: false } : {}),
  }));
  ...
  const [audioIntegrated, setAudioIntegrated] = useState(createAudioIntegratedState);
  ...
  const [live, setLive] = useState({
    ...
    savedPresetIds: [],
    activePreviewImage: "modern_white_meeting_room.png",
    presetSnapshots: {},
  });
  ...
  useEffect(() => {
    ...
    img.src = "modern_white_meeting_room.png?t=" + Date.now();
  }, []);
    ...
    <div id="aver-paint-look-root" style={{ position: "relative", background: T.page, width: "100%", height: "100vh", fontFamily: fUI, color: T.text, display: "flex", overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          <span style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{prototypeVersion.toUpperCase()}</span>
          ...
          <div className="aver-fade" style={{ margin: "8px 0 0", padding: "12px 18px 16px", borderTop: `1px solid ${T.line}`, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 0 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[["presenter", "Presenter"], ["audience", "Audience"], ["zone", "Zone"], ["hybrid", "Hybrid"], ["framing", "Framing"]].map(([id, lb]) => (
      ...
      <div id="aver-main-stage" style={{ position: "relative", flex: 1, padding: "16px 24px", minWidth: 0, background: T.page, overflow: "hidden", height: "100vh", display: "flex", flexDirection: "column", boxSizing: "border-box" }}>
        ...
        {activeMenu === "paint" ? (
                    ...
                    <div style={{ position: "relative", height: "100%", width: "auto", aspectRatio: "16 / 9", overflow: "hidden" }}>
                      <div
                        style={{
                          position: "absolute",
                          inset: 0,
                          backgroundImage: "url(modern_white_meeting_room.png)",
          ...
          (() => {
            const networkGap = 10;
            const section = { width: "100%", padding: 8, boxSizing: "border-box", background: T.panel, border: `1px solid ${T.line}`, borderRadius: 6 };
            const sectionGrid = { display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: networkGap, alignItems: "start" };
            const twoColumnCluster = { gridColumn: "span 2", display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: networkGap, alignItems: "start", minWidth: 0 };
            const card = { width: "100%", border: `1px solid ${T.line}`, borderRadius: 4, background: "#0d0f12", display: "flex", flexDirection: "column", boxSizing: "border-box", overflow: "hidden" };
            const head = { minHeight: 25, boxSizing: "border-box", display: "flex", alignItems: "center", background: T.panel2, padding: "0 10px", fontSize: 12.5, lineHeight: 1.2, fontWeight: 600, color: T.dim, borderBottom: `1px solid ${T.line}` };
            const body = { padding: "8px 10px 10px", display: "flex", flexDirection: "column", gap: 7, flex: 1 };
            const lab = { fontSize: 12, color: T.dim, marginBottom: 3, fontWeight: 600 };
            const inp = (val, on, dis) => ({ width: "100%", height: 30, boxSizing: "border-box", background: dis ? "#0a0b0c" : "#101216", border: `1px solid ${T.line2}`, borderRadius: 4, color: dis ? T.faint : T.text, fontSize: 13, padding: "5px 8px", fontFamily: fUI, outline: "none" });
            const Inp = ({ k, disabled, type = "text" }) => (<input type={type} value={net[k]} disabled={disabled} onChange={(e) => updNet(k, e.target.value)} style={inp(net[k], null, disabled)} />);
            const Radio2 = ({ k, opts = ["on", "off"], labels = ["On", "Off"], disabled = false }) => (
              <div style={{ display: "flex", alignItems: "center", gap: 22, minHeight: 28 }}>
                {opts.map((o, i) => (
                  <div key={o} aria-disabled={disabled} onClick={() => !disabled && updNet(k, o)} style={{ display: "flex", alignItems: "center", gap: 6, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? .45 : 1 }}>
                    <span style={{ width: 14, height: 14, borderRadius: "50%", border: `1.5px solid ${net[k] === o ? T.blue : T.line2}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {net[k] === o && <span style={{ width: 6, height: 6, borderRadius: "50%", background: T.blue }} />}
                    </span>
            ...
            const Btn = ({ children, primary, disabled, fill = false, onClick }) => (
              <button disabled={disabled} onClick={onClick} style={{ width: fill ? "100%" : undefined, minHeight: 30, padding: "5px 14px", fontSize: 13, fontWeight: 600, cursor: disabled ? "not-allowed" : "pointer", borderRadius: 4, border: `1px solid ${T.line2}`, background: disabled ? "#0d0f11" : "#1a1d21", color: disabled ? T.faint : T.text, fontFamily: fUI }}>{children}</button>
            );
            const sel = { width: "100%", height: 30, boxSizing: "border-box", background: "#101216", border: `1px solid ${T.line2}`, borderRadius: 4, color: T.text, fontSize: 13, padding: "4px 8px", fontFamily: fUI };
            const dhcpOn = net.dhcp === "on";
            const networkAddressDirty = ["ip", "netmask", "gateway", "dns"].some((key) => net[key] !== savedNetworkAddress[key]);
            const dot1xOn = net.dot1x === "on";
            const freeDOn = net.freeD === "on";
            const FilePick = ({ k, disabled }) => (
              <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
                <label style={{ minHeight: 30, boxSizing: "border-box", padding: "5px 10px", display: "inline-flex", alignItems: "center", borderRadius: 4, border: `1px solid ${T.line2}`, background: disabled ? "#0d0f11" : "#1a1d21", color: disabled ? T.faint : T.text, fontSize: 12.5, fontWeight: 600, cursor: disabled ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}>
                  Choose File<input type="file" disabled={disabled} hidden onChange={(e) => updNet(k, e.target.files?.[0]?.name || "")} />
                </label>
                <span title={net[k] || "No file selected"} style={{ minWidth: 0, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: disabled ? T.faint : T.dim, fontSize: 11.5 }}>{net[k] || "No file selected"}</span>
                <Btn disabled={disabled || !net[k]}>Upload</Btn>
              </div>
            );
            return (
              <div id="aver-network-wrapper" style={{ width: "min(calc(75vw - 40px), 100%)", marginLeft: "max(0px, calc(16.6667vw - 225.33px))", height: "100%", overflowY: "auto", paddingRight: 8, boxSizing: "border-box", display: "flex", flexDirection: "column", gap: networkGap }}>
                {/* Row 1: Network Configuration / NTP */}
                <div style={{ ...section, ...sectionGrid }}>
                  <div style={{ ...card, gridColumn: "span 2" }}>
                    <div style={head}>Network Configuration</div>
                    <div style={body}>
                      <div style={{ display: "grid", gridTemplateColumns: "minmax(150px, .8fr) minmax(0, 1.2fr)", gap: networkGap, alignItems: "end" }}>
                        <div><div style={lab}>DHCP</div><Radio2 k="dhcp" /></div>
                        <div><div style={lab}>Hostname</div><Inp k="hostname" /></div>
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 6, paddingTop: 7, borderTop: `1px solid ${T.line}` }}>
                      ...
                      <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "auto" }}>
                        <Btn disabled={dhcpOn || !networkAddressDirty} onClick={() => setSavedNetworkAddress({ ip: net.ip, netmask: net.netmask, gateway: net.gateway, dns: net.dns })}>Confirm</Btn>
                      </div>
                    </div>
                  </div>
                  <div style={card}>
                ...
                <div style={{ ...section, ...sectionGrid }}>
                  <div style={{ ...card, gridColumn: "span 2" }}>
                    ...
                    <div style={{ ...body, display: "grid", gridTemplateColumns: "minmax(0, 48fr) minmax(0, 52fr)", gap: networkGap, alignItems: "stretch" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: 7, minWidth: 0 }}>
                      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 7fr) minmax(86px, 3fr)", gap: 6 }}>
                        <div><div style={lab}>Destination IP</div><Inp k="srtIp" /></div>
                        <div><div style={lab}>Port</div><Inp k="srtPort" /></div>
                      </div>
                      <div><div style={lab}>Latency</div><div style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: "48%", minWidth: 110 }}><Inp k="srtLatency" /></div><span style={{ fontSize: 12, color: T.text }}>ms</span></div></div>
                      <div style={{ marginTop: "auto", fontSize: 12.5, color: T.dim }}>Connect Status: <span style={{ marginLeft: 6, color: T.faint }}>Disconnected</span></div>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 7, minWidth: 0 }}>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginTop: "auto" }}><Btn fill>Start Stream</Btn><Btn fill disabled>STOP</Btn></div>
                    </div>
                    </div>
                  </div>
                </div>
                ...
                {/* Row 6: 802.1X */}
                <div style={{ ...section, ...sectionGrid }}>
                  <div style={twoColumnCluster}>
                    <div style={card}><div style={head}>802.1X Enable</div><div style={body}><Radio2 k="dot1x" /></div></div>
                  <div style={card}><div style={head}>EAP Method</div><div style={body}><Radio2 k="eapMethod" opts={["md5", "tls", "peap"]} labels={["MD5", "TLS", "PEAP"]} disabled={!dot1xOn} /></div></div>
                  <div style={{ ...card, gridColumn: "1 / -1" }}>
                    <div style={head}>EAP Setting</div>
                    <div style={{ ...body, display: "grid", gridTemplateColumns: "1fr 1fr", gap: networkGap }}>
                      <div><div style={lab}>Identity</div><Inp k="eapIdentity" disabled={!dot1xOn} /></div>
                      <div><div style={lab}>Password</div><Inp k="eapPassword" type="password" disabled={!dot1xOn} /></div>
                    </div>
                  </div>
                  <div style={{ ...card, gridColumn: "1 / -1" }}>
                    <div style={head}>Client Certificate</div>
                    <div style={{ ...body, display: "grid", gridTemplateColumns: "1fr 1fr", gap: networkGap }}>
                      <div><div style={lab}>Import</div><FilePick k="eapClientFile" disabled={!dot1xOn} /></div>
                      <div><div style={lab}>Private Key Password</div><Inp k="eapPrivateKeyPassword" type="password" disabled={!dot1xOn} /></div>
                    </div>
                  </div>
                  <div style={card}><div style={head}>CA Certificate</div><div style={body}><div><div style={lab}>Import</div><FilePick k="eapCaFile" disabled={!dot1xOn} /></div></div></div>
                    <div style={{ alignSelf: "stretch", display: "flex", alignItems: "flex-end" }}><Btn disabled={!dot1xOn}>Confirm</Btn></div>
                  </div>
                </div>
                {/* Row 7: FreeD */}
                <div style={{ ...section, ...sectionGrid }}>
                  <div style={twoColumnCluster}>
                    <div style={card}><div style={head}>FreeD</div><div style={body}><Radio2 k="freeD" /></div></div>
                  <div style={card}><div style={head}>Camera ID</div><div style={body}><Inp k="freeDCameraId" disabled={!freeDOn} /></div></div>
                  <div style={card}><div style={head}>IP Address</div><div style={body}><Inp k="freeDIp" disabled={!freeDOn} /></div></div>
                  <div style={card}><div style={head}>Port</div><div style={body}><Inp k="freeDPort" disabled={!freeDOn} /></div></div>
                  <div style={card}><div style={head}>Pan Backlash</div><div style={body}><Inp k="freeDPanBacklash" disabled={!freeDOn} /></div></div>
                  <div style={card}><div style={head}>Tilt Backlash</div><div style={body}><Inp k="freeDTiltBacklash" disabled={!freeDOn} /></div></div>
                    <div style={{ gridColumn: "1 / -1", display: "flex", justifyContent: "flex-end" }}><Btn disabled={!freeDOn}>Confirm</Btn></div>
                  </div>
                </div>
              </div>
            );
          ...
          (() => {
            const card = { border: `1px solid ${T.line}`, borderRadius: 4, background: "#0d0f12", display: "inline-flex", flexDirection: "column", boxSizing: "border-box", alignSelf: "flex-start", overflow: "hidden" };
            const head = { minHeight: 25, boxSizing: "border-box", display: "flex", alignItems: "center", background: T.panel2, padding: "0 10px", fontSize: 12.5, lineHeight: 1.2, fontWeight: 600, color: T.dim, borderBottom: `1px solid ${T.line}`, whiteSpace: "nowrap" };
            const body = { padding: "8px 10px 10px", display: "flex", flexDirection: "column", gap: 7, flex: 1 };
            const inpStyle = { boxSizing: "border-box", height: 30, background: "#101216", border: `1px solid ${T.line2}`, borderRadius: 4, color: T.text, fontSize: 13, padding: "5px 8px", fontFamily: fUI, outline: "none", minWidth: colW(5) };
            ...
            const Btn = ({ children }) => (<button style={{ minHeight: 30, padding: "5px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer", borderRadius: 4, border: `1px solid ${T.line2}`, background: "#1a1d21", color: T.text, fontFamily: fUI, whiteSpace: "nowrap" }}>{children}</button>);
            ...
            const bigRow = { width: "100%", boxSizing: "border-box", display: "flex", flexWrap: "wrap", gap: 10, alignItems: "flex-start", padding: 8, background: T.panel, border: `1px solid ${T.line}`, borderRadius: 6 };
            ...
            return (
              <div id="aver-system-wrapper" style={{ width: "min(calc(75vw - 40px), 100%)", marginLeft: "max(0px, calc(16.6667vw - 225.33px))", height: "100%", overflowY: "auto", paddingRight: 8, boxSizing: "border-box", display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          (() => {
            ...
            const Btn = ({ children }) => (<button style={{ minHeight: 30, padding: "5px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer", borderRadius: 4, border: `1px solid ${T.line2}`, background: "#1a1d21", color: T.text, fontFamily: fUI }}>{children}</button>);
            // 大區塊外框(fill 左右寬度)
            const bigRow = { width: "100%", boxSizing: "border-box", display: "flex", flexWrap: "wrap", gap: 10, alignItems: "flex-start", padding: 8, background: T.panel, border: `1px solid ${T.line}`, borderRadius: 6 };
            const fieldLab = { fontSize: 12.5, color: T.dim, marginBottom: 5, fontWeight: 600 };
            return (
              <div id="aver-ndi-wrapper" style={{ width: "min(calc(75vw - 40px), 100%)", marginLeft: "max(0px, calc(16.6667vw - 225.33px))", height: "100%", overflowY: "auto", paddingRight: 8, boxSizing: "border-box", display: "flex", flexDirection: "column", gap: 10 }}>
                {/* 頂部:Built-in NDI */}
                <div style={{ ...bigRow }}>
          ...
          (() => {
            const TRK_TABS = [["presenter", "Presenter"], ["audience", "Audience"], ["zone", "Zone"], ["hybrid", "Hybrid"], ["framing", "Framing"], ["gesture", "Gesture"], ["face", "Face Enrollment"]];
              ...
              <div id="aver-tracking-wrapper" style={{ width: "min(1200px, 100%)", marginInline: "auto", height: "100%", minHeight: 0, overflow: "hidden", boxSizing: "border-box", display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div id="aver-trk-preview-row" style={{ flex: "1 1 0", minHeight: 0, display: "flex", gap: 10, alignItems: "stretch" }}>
                  <div id="aver-trk-preview-panel" style={{ flex: "1 1 0", minWidth: 0, position: "relative", borderRadius: 10, overflow: "hidden", border: `1px solid ${T.line}`, background: "#000", minHeight: 0 }}>
                    {trk.tab === "face" ? (
                      ...
                      <div data-tracking-background style={{ position: "absolute", inset: 0, backgroundImage: "url(modern_white_meeting_room.png)", backgroundSize: "cover", backgroundRepeat: "no-repeat", backgroundPosition: "center", transform: `translate(${ptz.pan}%, ${ptz.tilt}%) scale(${ptz.zoom * 1.65})`, transition: "transform 0.1s ease-out" }} />
                    )}
                ...
                {trk.tab === "presenter" ? (
                  ...
                  <div id="aver-presenter-panel" style={{ display: "flex", flexDirection: "column", gap: 8, height: "100%", minHeight: 0 }}>
                    <div className="tracking-mode-bar" style={{ ...sec, padding: "4px 0", gap: 18, border: "none", borderBottom: `1px solid ${T.line2}`, borderRadius: 0, background: "transparent", flexDirection: "row", flexWrap: "wrap", alignItems: "center", flexShrink: 0 }}>
                      {[
                        { key: "microphoneTracking", label: "Tracking Target", options: [[false, "Presenter"], [true, "Microphone Holder"]] },
                        { key: "multiPresenterTracking", label: "Presenter Mode", options: [[false, "Single Person"], [true, "Multiple People"]] },
                      ].map(({ key, label, options }) => (
                        <div key={key} style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: 8, position: "relative", flex: "0 0 auto", ...(key === "multiPresenterTracking" ? { borderLeft: `1px solid ${T.line2}`, paddingLeft: 18 } : {}) }}>
                          <span className="tracking-mode-title" style={{ ...secTitle, margin: 0 }}>{label}</span>
                    ...
                    <div className="presenter-v2-columns" style={{ display: "grid", gridTemplateColumns: "1fr 1.1fr 1fr", gap: 10, alignItems: "stretch", flex: 1, minHeight: 0 }}>
                      ...
                      <section className="presenter-compact-section" aria-label="Framing">
                        ...
                        <div className="presenter-form-row"><label htmlFor="v2-tracking-point">Tracking Point</label><div style={{ display: "flex", gap: 6, minWidth: 0 }}><input id="v2-tracking-point" value={trk.presetPoint} onChange={e => updTrk("presetPoint", e.target.value)} style={{ ...sel, minWidth: 0 }} /><button style={{ ...secondaryBtn, padding: "4px 10px" }}>Save</button></div></div>
                        {[
                          ["peopleSize", "People Size", ["Upper Body", "Full Body", "Close Up"]],
                          ["placement", "Placement", ["Center", "Left", "Right"]],
                          ["height", "Height", ["Height1", "Height2"]],
                        ].map(([key, label, values]) => <div key={key} className="presenter-form-row"><label htmlFor={`v2-${key}`}>{label}</label><FramingSelect kind={key} label={label} value={trk[key]} options={values} onChange={value => updTrk(key, value)} /></div>)}
                        <div style={{ display: "flex", alignItems: "center", gap: 12, minHeight: 28 }}><TrkCheck stateKey="autoZoom" label="Auto Zoom" /><TrkCheck stateKey="autoTilt" label="Auto Tilt" /><button type="button" aria-label="Auto Zoom information" onClick={() => { setPresenterHelp({ title: "Auto Zoom", text: "When Auto Zoom is off, camera stops zooming in/out automatically and shoots the presenter according to the shot size of the preset you choose." }); presenterHelpRef.current?.showModal(); }} style={{ border: "none", background: "transparent", color: T.dim, cursor: "pointer" }}>ⓘ</button></div>
                        <div className="presenter-form-row"><label htmlFor="v2-zoom-preset">Zoom Preset</label><select id="v2-zoom-preset" value={trk.autoZoomPreset} onChange={e => updTrk("autoZoomPreset",e.target.value)} style={sel}><option>Preset 1</option><option>Preset 2</option><option>Preset 3</option></select></div>
                      </div></section>
                      <section className="presenter-compact-section" aria-label="Tracking Behavior">
                        <div style={{ ...secTitle, flexShrink: 0, padding: "8px 10px", margin: 0, borderBottom: `1px solid ${T.line}` }}>Tracking Behavior</div><div className="presenter-card-body">
                        {[["sensitivity", "Sensitivity", "Tracking Sensitivity",1,3],["returnTime","Return Time","Time of Return to Tracking Point",3,10]].map(([key,label,full,min,max]) => <div key={key} className="presenter-form-row"><label htmlFor={`v2-${key}`} title={full}>{label}</label><div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}><span style={{ color: T.faint, fontSize: 11 }}>{min}</span><input id={`v2-${key}`} aria-label={full} type="range" min={min} max={max} value={trk[key]} onChange={e => updTrk(key,Number(e.target.value))} className="tr-sl" style={{ "--p": ((trk[key]-min)/(max-min))*100+"%", flex: 1, minWidth: 0, margin: 0 }} /><span style={{ color: T.faint, fontSize: 11 }}>{max}</span><output style={{ color: T.blue, fontFamily: fMono, minWidth: 18, textAlign: "right" }}>{trk[key]}</output></div></div>)}
                        <div style={{ display: "flex", flexDirection: "column", gap: 6, paddingTop: 8, borderTop: `1px solid ${T.line}`, opacity: trk.microphoneTracking ? 0.4 : 1 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}><label htmlFor="v2-detection">Multi-Presenter Detection</label><button disabled={trk.microphoneTracking} type="button" aria-label="Multi-Presenter Detection information" onClick={() => { setPresenterHelp({ title: "Multi-Presenter Detection", text: "When two or more people appear, the camera moves to the Multi-Person Preset Point to include everyone. Select a preset point wide enough to cover the scene." }); presenterHelpRef.current?.showModal(); }} style={{ border: "none", background: "transparent", color: T.dim, cursor: trk.microphoneTracking ? "not-allowed" : "pointer" }}>ⓘ</button></div>
                          <select id="v2-detection" aria-label="Multi-Presenter Detection" disabled={trk.microphoneTracking} value={trk.multiPresenter} onChange={e => updTrk("multiPresenter",e.target.value)} style={{ ...sel, cursor: trk.microphoneTracking ? "not-allowed" : "pointer" }}><option value="off">Off</option><option value="preset1">Preset 1</option><option value="preset2">Preset 2</option></select>
                        </div>
                      </div></section>
                    </div>
                  ...
                  <AudienceSettings settings={audienceSettings} onChange={setAudienceSettings} theme={T} visualTargets={prototypeVersion === "v3"} />
                ) : trk.tab === "zone" ? (
          ...
          <AudioIntegratedPanel value={audioIntegrated} onChange={setAudioIntegrated} onNotify={flash} />
        ) : (
          <div id="aver-video-audio-wrapper" style={{ display: "flex", flexDirection: "column", gap: 10, width: "min(calc(75vw - 40px), 100%)", marginLeft: "max(0px, calc(16.6667vw - 225.33px))", height: "100%", overflowY: "auto", paddingRight: 8, boxSizing: "border-box" }}>
            
            {/* Video & Audio 設置區容器 */}
            <div style={{ display: "flex", flexDirection: "column", gap: 10, width: "100%" }}>
              
              {/* 頂部的三個獨立 FormField 欄位，寬度與下方 Stream Video Output 相同 */}
              <div style={{ width: "100%", padding: 8, boxSizing: "border-box", background: T.panel, border: `1px solid ${T.line}`, borderRadius: 6, display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 10, alignItems: "stretch" }}>
                {/* Power Frequency */}
      ...
      {gridDebug === "layout" && <LayoutInspector />}
      {/* ===== 多段 Grid Debug 網格覆蓋層 ===== */}
      ...
      <button
        id="aver-layout-inspector-toggle"
        onClick={() => setGridDebug((prev) => prev === "layout" ? "off" : "layout")} 
        title="版面比例檢查模式"
        style={{ 
          position: "absolute", 
          left: 10, 
          ...
          height: 26, 
          borderRadius: 6, 
          border: `1px solid ${gridDebug === "layout" ? "#5edfff" : T.line2}`, 
          background: gridDebug === "layout" ? "rgba(94,223,255,0.18)" : "rgba(22,24,27,0.85)", 
          color: gridDebug === "layout" ? "#5edfff" : T.faint, 
          fontSize: 10, 
          fontWeight: 700, 
          cursor: "pointer", 
          fontFamily: fMono,
