...
export default function AudienceV1({ settings: s, onChange, theme: T, designBranch = 0 }) {
  ...
  return <div id="aver-v1-audience-panel" style={{ height: '100%', minHeight: 0, display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gridTemplateRows: headerTarget ? 'auto repeat(2,minmax(0,1fr)) 49.265625px 67.96875px' : designBranch === 2 ? 'repeat(2,minmax(0,1fr)) auto auto' : 'repeat(2,minmax(0,1fr)) auto minmax(0,1fr)', alignContent: 'stretch', gap: headerTarget ? 8 : 6, columnGap: 8, fontSize: 13, color: T.text }}>
    ...
    <div style={{ ...box, gridColumn: 2, gridRow: shifted ? 3 : firstDetailRow, alignSelf: 'start' }}><label htmlFor="v1-audience-preset" style={title}>Tracking Point</label><div style={{ display: 'flex', gap: 8 }}><input id="v1-audience-preset" value={s.presetPoint} onChange={e => update('presetPoint',e.target.value)} style={field} /><button style={button} onClick={() => { update('savedPoint',{preset:s.presetPoint,peopleSize:s.peopleSize,placement:s.placement,height:s.height});setSaved(true); }}>Save</button></div>{saved && <span role="status" style={title}>Tracking point saved.</span>}</div>
    <div style={{ ...box, gridColumn: 2, gridRow: shifted ? 4 : firstDetailRow + 1, alignSelf: 'start', display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: 8 }}>{[['peopleSize','People Size',['Upper Body','Full Body','Close Up']],['placement','Placement',['Center','Left','Right']],['height','Height',['Height1','Height2']]].map(([key,label,options]) => <div key={key}><label htmlFor={`v1-audience-${key}`} style={title}>{label}</label><select id={`v1-audience-${key}`} value={s[key]} onChange={e => update(key,e.target.value)} style={{ ...field, marginTop: 6 }}>{options.map(value => <option key={value}>{value}</option>)}</select></div>)}</div>
